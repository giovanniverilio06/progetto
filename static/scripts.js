window.onload = function () {
    ...
    function loadRecentPerizieDashboard(perizie) {
        ...
        if (sortedPerizie.length === 0) {
            tableBody.innerHTML = `
                <tr>
                    <td colspan="7" class="text-center py-3">Non ci sono nuove perizie in attesa</td>
                </tr>
            `;
            return;
        }
            ...
            <div class="card h-100 shadow-sm ${cardBackground}">
                <div class="card-body text-center">
                    ...
                    <div class="mb-3">
                        ...
                        <span class="badge ${isOccupato ? 'bg-danger' : 'bg-success'} ms-1">
                            ${isOccupato ? 'Occupato' : 'Disponibile'}
                        </span>
                    </div>
                    <div class="d-flex justify-content-center">
                        <button class="btn btn-sm btn-info me-2 view-operator" data-id="${operatore.username}">
...
function addOperatorButtonListeners() {
    ...
    // Assign button
    document.querySelectorAll('.assign-operator').forEach(button => {
        button.addEventListener('click', function () {
            const operatorId = this.getAttribute('data-id');
            assignPerizie(operatorId);
        });
    });
